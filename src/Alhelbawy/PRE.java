	  ...
	  //   System.out.println(graphnode);
	    	List<List<Double>>nodelist=tm.listToDouble(nodearray, regraph);
	  	   //  System.out.println(nodelist);
	  	    ...
	  	    List<Double> prlist=PR.calPageRank(nodelist, onelist,norlist,PR.ALPHA);
	  	    // PR.printVec(prlist);
	  	     //计算节点与整个输入文本的相关度
	     ...
	     */
	  	  ...
	  	  Dynamic dy=new Dynamic();
	  	   for(int i=0;i<prlist.size();i++){
	  	     ...
	  	     double scon1=Iconlist.get(i)+prlist.get(i);;
	  	     candite.put(nodename, scon);
	  	     candite1.put(nodename,scon1);
	  	     }
	  //	     System.out.println(candite);
	  	   for(int i=0;i<entitylist.size();i++){
				 String mention=entitylist.get(i);
				 HashMap<String,Double>phash=new HashMap<String,Double>();
				 HashMap<String,Double>phash1=new HashMap<String,Double>();
				 for(String key:candite.keySet()){
					 //获取实体指称项对应的候选实体的可信度
					 if(mention.equals(pd.getMen(key))){
						 phash.put(pd.getCan(key), candite.get(key));
						 phash1.put(pd.getCan(key), candite1.get(key));
					 }
				 }//emd for
				 
				//在此处使用动态选择算法
				String correctcandite=dy.dynamic(phash, phash1, wikiurl);
				result.put(mention, correctcandite);
		  ...
		  try {
		         ...
		         while((str=br.readLine())!=null) {
		          ...
		          PRE pre=new PRE();
		           ...
		           HashMap<String,String>disresult=pre.linking(text,tagger,wiki,classifier);
